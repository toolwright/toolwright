"""GovernanceEngine — transport-agnostic request pipeline.

Encapsulates the full tool-call lifecycle as a reusable pipeline that any
transport (stdio, HTTP, CLI, REST) can invoke:
...
class GovernanceEngine:
    """Transport-agnostic request governance pipeline for tool calls.

    Evaluates approval, behavioral rules, circuit breakers, and confirmation
    gates before executing any tool call. Usable by any transport adapter.

    Formerly known as RequestPipeline.
    """
    ...
    def _emit_console_event(
        ...
        from toolwright.core.governance.event_store import ConsoleEvent
...
RequestPipeline = GovernanceEngine
